...
// Runtime hero image discovery — scans /photos/hero/ directory
async function discoverHeroImages(): Promise<string[]> {
  // Common hero image filenames to check (case-insensitive, deduped)
  const candidates = [
  ...
  const exists = await Promise.all(
    candidates.map(async (name) => {
      try {
        const res = await fetch(`/photos/hero/${name}`, { method: 'HEAD' });
        return res.ok ? `/photos/hero/${name}` : null;
      } catch {
        return null;
      }
    })
  ...
  // Deduplicate by lowercase filename to avoid case-variant duplicates
  const seen = new Set<string>();
  return exists.filter((url): url is string => {
    if (!url) return false;
...
export default function HeroSlideshow({ dark }: Props) {
  ...
  useEffect(() => {
    ...
    async function load() {
      // In development, use Vite's glob results directly
      if (import.meta.env.DEV) {
        const { getHeroUrls } = await import('../imageConfig');
        const urls = getHeroUrls();
        if (!cancelled && urls.length > 0) setSlides(urls);
        return;
      }

      // In production, discover images at runtime
      const urls = await discoverHeroImages();
      if (!cancelled && urls.length > 0) setSlides(urls);
    }
